...
#     mutate(efficiency = amount / total_cost)
#   # start by including free planning units and those with status = 2
#   # Marxan adds cost of all status = 2 units to penalty regardless of whether
#   # or not species occurs in unit
#   fixed_cost <- subset(pu, status == 2) %>% 
#     {sum(.$cost + blm * .$boundary)}
#   p <- filter(remaining, status == 2 | total_cost == 0) %>% 
#     group_by(feature) %>% 
#     summarize(cumulative_amount = sum(amount)) %>% 
#     right_join(dplyr::select(features, feature, target), by = "feature") %>% 
...
I run the Marxan emulator for 100 simulated annealing repetitions with 10,000 iterations each. 

```{r run-emulator}
results <- marxan_emulator(pu, species, target_prop, spfs,
                           blm = best_blm, nrep = 1, n = 10000)
```

Now I map the best reserve, i.e. the one with the lowest objective function:

```{r map-emulator}
map_selection <- function(x, pu) {
  ...
  spplot(pu, "selected", col = "grey20", col.regions = status_pal, 
         colorkey = list(space = "bottom", height = 0.5)) %>% 
    print
}
best <- which.max(results$summary$cost)
x <- filter(results$selections, run == best) %>% 
  {.$x} %>% 
  map_selection(results$pu)
```

And the selection frequency, which is the frequency with which each planning unit was selection and gives a sense for the imporantance of each planning unit.

```{r map-emulator-sf}
map_frequency <- function(selections, pu) {
  pu <- group_by(selections, pu) %>% 
    summarize(frequency = sum(x) / n()) %>% 
    rename(id = pu) %>% 
    merge(pu, ., by = "id")
  pu$frequency[pu$status == 3] <- NA
  spplot(pu, "frequency", main = "Selection Frequency",
         col.regions = viridis(100),
         at = seq(0, 1.01, len = 100))
}
map_frequency(results$selections, pu)
```

## Marxan

Now I perform a normal Marxan run, with identical paramters, which I will compare to my R emulation. The R package `marxan` makes running Marxan from within R a breeze and has some great vignettes: `browseVignettes("marxan")`.

```{r mardan-data}
mr <- marxan(pu, species,
             targets = results$features$target,
             spf = results$features$spf,
             BLM = 10, PROP = 0, VERBOSITY = 5L,
             NUMREPS = 10L, NUMITNS = 10000L, NUMTEMP = 1000L)
```
...
Finally, I compare the results to demonstrate that the emulator I've created functions essentially identically to Marxan. I'll compare the two methods with respect to several different components
...
mr@results@summary[mr@results@best,]$Connectivity_Total
sum(results$boundary$boundary)

comp <- mr@results@selections[mr@results@best,] %>% 
  results$objective(breakdown = TRUE)
comp
mr@results@summary[mr@results@best,]
comp$cost
mr@results@summary[mr@results@best,]$Cost

comp$shortfall
mutate(mr@results@summary, zero = Score - Cost - 10 * Connectivity - Penalty)
         

mr@results@summary[mr@results@best,]$Penalty
sf_pen <- rev(c(5783.12, 7020.91, 6767.03, 5813.69))
bp <- base_penalty(pu, puvspr, results$features, blm = 10)
blank <- which(init_res(pu, 0))
filter(puvspr, pu %in% blank) %>% 
  group_by(feature) %>% 
  summarise(amount = sum(amount)) %>% 
  right_join(results$features, by = "feature") %>% 
  mutate(amount = ifelse(is.na(amount), 0, amount),
         mpm = (1 - amount / target)) %>% 
  inner_join(bp, by = "feature") %>% 
  mutate(sf_pen = mpm * spf * penalty) %>% 
  summarize(sf_pen = sum(sf_pen))
sum((1 - mr@results@mpm[mr@results@best,]) * bp$penalty)
sum((1 - mr@results@mpm[mr@results@best,]) * sf_pen)

results$objective(init_res(pu, prop = 0))
sum(ofun(init_res(pu, prop = 0))$shortfall)
```