...
mr <- marxan(pu, species,
             ...
             spf = results$features$spf, #c(7, 7, 12, 17),
             BLM = 10, PROP = 0, VERBOSITY = 3L,
             NUMREPS = 10L, NUMITNS = 100000L, NUMTEMP = 10000L)
...
mu <- MarxanUnsolved(mr@opts, mr@data)
dir <- "~/Desktop/marxan/"
write.MarxanUnsolved(mu, dir)
file.copy(options()$marxanExecutablePath,
          file.path(dir,basename(options()$marxanExecutablePath)))
paste0('"',file.path(dir, basename(options()$marxanExecutablePath)),
    '" "',file.path(dir, 'input.dat'),'"')
system()
...
```{r}
base_penalty(results$pu, puvspr, results$features, blm = 10)
...
1 - mr@results@occheld[mr@results@best,] / mr@data@species$target
sum((1 - mr@results@mpm[mr@results@best,]) * c(9586.12, 12481.3, 12301.5, 9908.35)) / 2

mr@results@occheld[mr@results@best,]


sf_pen <- c(9586.12, 12481.3, 12301.5, 9908.35)
sum((1 - mr@results@mpm[mr@results@best,]) * c(9586.12, 12481.3, 12301.5, 9908.35))
mr@results@mpm[mr@results@best,]
16349.02 - 10476.44 - 266.7734
```
