...
Amazon's **Simple Storage Service (S3)** offers [extremely cheap](https://aws.amazon.com/s3/pricing/) (~$0.03 per TB per month), highly scalable cloud-based storage of objects of almost any size. In S3, data (i.e. files) are bundled together with metadata into a [**objects**](https://en.wikipedia.org/wiki/Object_storage), and objects are organized into **buckets**. Buckets are given unique names and objects are given unique.

## Install and configure the AWS CLI

To use S3 you'll need to install the Amazon Command Line Interface on your local machine. For Linux and Mac OS X users, run the following commands in the Terminal.
...
Now you'll need to configure the AWS CLI to interact with AWS. For this, you'll need the `credentials.csv` file you downloaded when [creating an IAM user](http://strimas.com/r/rstudio-cloud-1/#create-iam-user) at the start of part I of this series. If you can't find this file, you'll need to [create a new access key](http://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html#Using_CreateAccessKey) and download a new `credentials.csv` file.



## Create a bucket

On S3, files are stored in buckets. 

Once you have an RStudio project that you've pushed to GitHub, point your browser to your EC2 instance and log on to RStudio Server ([covered in part I](http://strimas.com/r/rstudio-cloud-1/)).
