...
```{r run-emulator-real, echo=F}
f_results <- "data/marxan/emulator-results.rds"
if (!file.exists(f_results)) {
  res_emulator <- marxan_emulator(pu, species, target_prop, spfs,
                           blm = best_blm, nrep = 100, n = 10000)
  saveRDS(res_emulator, f_results)
}
res_emulator <- readRDS(f_results)
```

```{r run-emulator-dummy, eval=F}
res_emulator <- marxan_emulator(pu, species, target_prop, spfs,
                                blm = best_blm, nrep = 100, n = 10000)
```
...
```{r adaptive-check-marxan, results="hide"}
...
```{r adaptive-check-emulator, dev='svg', fig.width=700/96, fig.height=350/96}
...
```{r side-by-side, fig.width=700/96, fig.height=350/96}
...
So, as expected, the performance is terrible. Marxan is `r round(unname(time_emulator / time_marxan)[1])` times faster!
