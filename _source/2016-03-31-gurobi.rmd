---
layout: post
title: "Systematic Reserve Design using the Gurobi Optimizer in R"
published: true
excerpt: >
  Solving Marxan-like systematic conservation planning problems using Integer 
  Linear Programming and Ineger Quadratic Programming techniques via the R
...
In the above equation, the first three terms comprise the objective function. The first term is just the total cost of a given candidate reserve. The second and third terms together give the total length of the perimeter of the reserve network. Note that in all the Marxan documentation I've seen the third term is absent; however, in the actual object function that Marxan uses this term is usually included. The goal is to minimize the objective function, and the final piece of the equation states that we want to do this subject to the constraint that targets are met for all conservation features.
...
In general, the **parameter space** (the set of all possible decision variable combinations) of these problems is so large that an exhaustive search of all possible sets of decision variables is impossible. For example, in a simple reserve design problem with 100 planning units, there are \\( 2^{100} \sim 10^{30} \\) possibilities in the parameter space. Even if evaluating the objective function for each set of decision variables only took a nanosecond, it would still take about 1,000 times the age of the universe to do so for all possibilities!
...
```{r set-targets}
set_targets <- function(x, features, type = c("proportional", "absolute")) {
  type <- match.arg(type)
  assert_that(inherits(features, "RasterStack"),
              is.numeric(x),
              length(x) == 1 || length(x) == nlayers(x))
...
Before we get started you'll need to install Gurobi. First, [download the Gurobi Optimizer](http://user.gurobi.com/download/gurobi-optimizer). Then [request an academic license](http://user.gurobi.com/download/licenses/free-academic). At this point you'll get an email with instructions for downloading and activating the license. If you aren't eligible for an academic license, you're probably out of luck; I don't know how much Gurobi costs, but suspect it's expensive.
...
The optional second argument, `params`, is a named list of components specifying Gurobi parameters. The full list of possible parameters is on available in [the Gurobi documentation](https://www.gurobi.com/documentation/6.5/refman/parameters.html#sec:Parameters); however, are few of potential relevance are:
...
gurobi_solve <- function(features, cost, targets, blm, edge_factor = 1,
                         target_type = c("proportional", "absolute"),
...
results <- gurobi_solve(species, cost, targets = 0.3, blm = 100)
best <- r
r[] <- results$x
...
cellStats(cost * r, "sum") + 100 * gLength(r_poly)
results$objval
```
