...
published: true
excerpt: >
  A field guide to various open-source integer linear programming solvers and
  and their corresponding R packages. The focus is on finding an open-source
  alternative to Gurobi for conservation prioritization and proteced area design.
...
`Rglpk` finds the correct optimal solution, and does so quickly. As with `Rglpk`, I've had to extract the lower bound from the screen output. I compare it to the Gurobi lower bound to demonstrate that Gurobi gives a better approximation.
