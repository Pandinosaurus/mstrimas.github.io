...
relaxed_symphony <- function(cost, rij, targets) {
...
msc_symphony <- function(cost, rij, targets,
  ...
  t <- system.time({
    ...
    relaxed <- relaxed_symphony(cost, rij, targets)
...
msc_glpk <- function(cost, rij, targets,
  ...
  # write to mps format
  if  (!is.null(filename)) {
    glpkAPI::writeMPSGLPK(model, glpkAPI::GLP_MPS_FILE, filename)
  }
  ...
  #glpkAPI::solveSimplexGLPK(model)
  # gap to optimality
...
comp_perf <- readRDS("data/ilp-performance/comp-perf.rds")
```

Let's take a look at the results:

```{r performance-results, fig.width=600/96, fig.height=600/96}
comp_perf %>% 
