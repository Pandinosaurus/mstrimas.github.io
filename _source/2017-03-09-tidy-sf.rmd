...
excerpt: >
  ...
  objects are stored as data frames and how this allows them to play nicely with 
...
nc %>% 
  ...
  # pick top three counties based on area
  slice(1:3)
```
...
It is also possible to use functions from `sf` that act on the geometry column within a mutate statement. For example, if there wasn't already an area column, one could be created with:
...
Note that this currently throws an error, but [I've filed an issue](https://github.com/edzer/sfr/issues/42) and I suspect it will be fixed soon.
