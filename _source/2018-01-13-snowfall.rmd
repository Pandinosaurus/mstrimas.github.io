...
```{r snow-data-cache, echo = FALSE}
r_snowfall <- here("_source/data/snowfall//snowfall_to_20180112.grd") %>% 
  stack()
layer_dates <- names(r_snowfall) %>% 
  str_extract("[0-9]{8}") %>% 
  ymd()
...
```{r hillshade, echo = -1, fig.width=960/96, fig.height=600/96}
par(mar = c(0, 0, 0, 3), bg = "grey20")
...
```{r animate-large, echo = FALSE, eval = FALSE}
setwd(here("img", "snowfall"))
f_animation <- here("img", "snowfall", "snowfall_to_20180112_fullsize.gif")
ani.options(interval = 0.1)
